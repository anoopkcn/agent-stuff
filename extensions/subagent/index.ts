...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("subagents", {
		...
		handler: async (args, ctx) => {
			...
			// Default to "both" for listing UX (tool defaults to "user" for safety — intentional difference)
			const scope: AgentScope = (["user", "project", "both"] as const).includes(rawScope as AgentScope)
				? (rawScope as AgentScope)
				: "both";

			const { agents } = discoverAgents(ctx.cwd, scope);

			if (agents.length === 0) {
				ctx.ui.notify(`No subagents found (scope: ${scope})`, "info");
				return;
			}
			...
			if (index !== -1) {
				...
				if (agent) ctx.ui.notify(agent.filePath, "info");
			}
		},
	});
}
