...
function sliceByColumn(text: string, start: number, end: number, _ansiAware = false): string {
	const width = Math.max(0, end - start);
	if (width <= 0) return "";
	if (visibleWidth(text) <= width) return text;

	// Keep the right edge when callers pass a non-zero start to trim from the left.
	// This is a small local replacement for the older pi-tui helper.
	if (start > 0) {
		const chars = Array.from(text);
		let out = "";
		for (let i = chars.length - 1; i >= 0 && visibleWidth(out) < width; i--) {
			out = chars[i] + out;
		}
